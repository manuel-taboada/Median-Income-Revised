...
************parámetros a ajustar tci tasa capital imputado
	...
	******************** Globales: agregados para ajustes a cuentas nacionales y registros administrativos;
	global indep_cn = 18205394588451.1/`ipc';
  global capital_CN = 21544735360503.1/`ipc';
	global rec_sii = 3616099000000/`ipc';
	    
	global sal_cn = 61107054012032.7/`ipc';
	global cot_dep = 2188575000000/`ipc'; 

    global cot_indep = 63914000000/`ipc';
...
ineqdeco y_bbb [w=expr] if  y_bbb>0 
	...
	putexcel L6 =`r(gini)', overwritefmt
	
sum y_bbb [w=expr] if  y_bbb>0, detail

	putexcel H6 =`r(mean)', overwritefmt
    putexcel I6 =`r(p50)', overwritefmt

	
drop aux*
replace y_gc1=y_gc1*`ipc'
bfmcorr using "C:\Users\manue\Documents\Magister\De Gregorio\Tesis\Bases\Encuestas\Finales\Segunda etrega\Replicabilidad\CASEN\gpinter 2015.xlsx", weight(expr) income(y_gc1) households(id_vivienda) taxunit(i) merg(${p_bfm} ) holdmar(dep cotiza trabajador contrib)
...
egen y_bbcn=rsum(y_b1cn y_b2cn y_b3cn y_bb4)
	...
	*****************REPORTAR FACTORES
	...
	putexcel f1 = ${year_casen} , overwritefmt
	sum f_recigc
	putexcel f2 =`r(mean)', overwritefmt
	sum f_cots
	putexcel f3 =`r(mean)', overwritefmt
	sum f_cots_i
	putexcel f4 =`r(mean)', overwritefmt
	sum f_rem
	putexcel f5 =`r(mean)', overwritefmt
	sum f_indep
	putexcel f6 =`r(mean)', overwritefmt
	sum f_cap
	putexcel f7 =`r(mean)', overwritefmt
	sum f_recigc_bfm
	putexcel f8 =`r(mean)', overwritefmt
	sum f_rembf
	putexcel f9 =`r(mean)', overwritefmt
	sum f_indepbf
	putexcel f10 =`r(mean)', overwritefmt
	sum f_capbfm
	putexcel f11 =`r(mean)', overwritefmt
	sum f_cotsbf
	putexcel f12 =`r(mean)', overwritefmt
	sum f_cots_ibf
	putexcel f13 =`r(mean)', overwritefmt
...
ineqdeco y_bbcn [w=cweight] if  y_bbcn>0 
	...
	putexcel M6 =`r(gini)', overwritefmt
	
sum y_bbcn [w=cweight] if  y_bbcn>0, detail

	putexcel J6 =`r(mean)', overwritefmt
    putexcel K6 =`r(p50)', overwritefmt
