...
************parámetros a ajustar tci tasa capital imputado
	...
	******************** Globales: agregados para ajustes a cuentas nacionales y registros administrativos;
	global indep_cn = 15727761202918.7/`ipc';
  global capital_CN = 21544735360503.1/`ipc';
	global rec_sii = 2799307600000/`ipc';
	    
	global sal_cn = 52768602707581.3/`ipc';
	global cot_dep = 1906340000000/`ipc'; 

    global cot_indep = 62633000000/`ipc';
...
use "casen_2013_mn_b_principal"


keep folio o expr o17 o14 o15 o29 edad y* activ numper  pco1
drop if edad<20
...
* Generamos el ingreso de los independientes
	...
	* egen aux1 = rsum(y260201c y2603c  yinv01 yinv03 ymon yorf yotp)
	egen aux3 = rsum(y2009 y2010 y2603 y2701c yinv ypbs ymon yorf yotp)
	replace y_n4 = aux3
...
replace y_gc1=y_gc1*`ipc'
	bfmcorr using "C:\Users\manue\Documents\Magister\De Gregorio\Tesis\Bases\Encuestas\Finales\Segunda etrega\Replicabilidad\CASEN\gpinter 2013.xlsx", weight(expr) income(y_gc1) households(folio) taxunit(i) merg(${p_bfm} ) holdmar(dep cotiza trabajador contrib)
replace y_gc1=y_gc1/`ipc'
...
egen y_bbcn=rsum(y_b1cn y_b2cn y_b3cn y_bb4)
	...
	*****************REPORTAR FACTORES
	...
	putexcel e1 = ${year_casen} , overwritefmt
	sum f_recigc
	putexcel e2 =`r(mean)', overwritefmt
	sum f_cots
	putexcel e3 =`r(mean)', overwritefmt
	sum f_cots_i
	putexcel e4 =`r(mean)', overwritefmt
	sum f_rem
	putexcel e5 =`r(mean)', overwritefmt
	sum f_indep
	putexcel e6 =`r(mean)', overwritefmt
	sum f_cap
	putexcel e7 =`r(mean)', overwritefmt
	sum f_recigc_bfm
	putexcel e8 =`r(mean)', overwritefmt
	sum f_rembf
	putexcel e9 =`r(mean)', overwritefmt
	sum f_indepbf
	putexcel e10 =`r(mean)', overwritefmt
	sum f_capbfm
	putexcel e11 =`r(mean)', overwritefmt
	sum f_cotsbf
	putexcel e12 =`r(mean)', overwritefmt
	sum f_cots_ibf
	putexcel e13 =`r(mean)', overwritefmt
	
putexcel set resultados_casen_stata.xlsx, sheet(Figure 10) modify
	
ineqdeco y_bbcn [w=cweight] if  y_bbcn>0 

	putexcel M5 =`r(gini)', overwritefmt
	
sum y_bbcn [w=cweight] if  y_bbcn>0, detail

	putexcel J5 =`r(mean)', overwritefmt
    putexcel K5 =`r(p50)', overwritefmt
