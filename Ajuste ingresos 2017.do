...
**Se seleccionan los párametros para el capital imputado k_i y el percentil desde el que se ajustan los datos con el registro de impuestos;
	...
	*#delimit ;
	...
	global tramo7_sc = 100000000000000;
...
************parámetros a ajustar tci tasa capital imputado
	...
	* Nombre del identificador de hogar;
	...
	global firmo 		=	"o17==1";
	global no_firmo 	=	"o17!=1 & o17!=.";
	global no_boleteo	= 	"o14!=1 & o14!=2";
	global da_boleta	=	"o14==1 | o14==2";	
	global no_tiene_afp	=	"y26_3b_in == 2 | y26_3b_in== 3 | y26_3c_in == 2 | y26_3c_in == 3 | y26_3g_in== 2 | y26_3g_in == 3";
	global cot_pension	=	"o29 >= 1 & o29 <= 6" ;
	...
	******************** Globales: agregados para ajustes a cuentas nacionales y registros administrativos;
	 global indep_cn = 20499648832980.4/`ipc';
  global capital_CN = 23067762485553/`ipc';
	global rec_sii = 4275072700000/`ipc';
   	global sal_cn = 68893350988082.7/`ipc';
	global cot_dep = 2553915578000/`ipc'; 

    global cot_indep = 73642537000/`ipc';
...
keep folio o id_vivienda hogar expr o17 o14 o15 o29 edad y* activ dau qaut numper  pco1
...
* Generamos el ingreso de los independientes
	...
	* egen aux1 = rsum(y260201c y2603c  yinv01 yinv03 ymon yorf yotp)
	egen aux3 = rsum(y260201c y2603c  yinv01 yinv02 yinv03 ymon yorf yotp)
	replace y_n4 = aux3
...
******generamos ingreso sujeto a IGC, neto
	...
	*********cuenta propia total
	...
	putexcel set resultados_casen_stata.xlsx, sheet(PEN Curve ${year_casen}) modify
		*******Brutificamos el ingreso sujeto al IGC
		
		
		
		egen double tot_w = total(expr) if y_n1>0&dep==1
		gen aux99 = expr if y_n1>0&dep==1
		
		sort y_n1 
		
		gen cum_w =sum(aux99) if y_n1>0&dep==1
		
		gen cumfreq=cum_w/tot_w
		
		egen cutter =cut(cumfreq), at(0(0.01)0.99 1)
		replace cutter=cutter[_n-1] if missing(cutter) &  y_n1>0&dep==1
		gen per=cutter
		
		preserve
		collapse (min) p = per (mean) y_n1 (count) obs = aux99 [fweight=expr] if y_n1>0 &  dep==1, by(cutter)
		 
		export excel using "C:\Users\manue\Documents\Banco Central\Trabajo\tesis\Replicability\CASEN\2020\penn_2017.xlsx", firstrow(variables) sheet("neto casen", modify)
...
replace cot=0 if (${no_tiene_afp}|~${cot_pension}|${no_firmo})
...
replace cot_i=0 if (${no_tiene_afp}|~${cot_pension}| ${no_boleteo})
...
***************************************************************************
 ...
 drop tot_w aux99 cum_w cumfreq cutter per
		
		egen double tot_w = total(expr) if y_bb1>0&dep==1
		gen aux99 = expr if y_bb1>0&dep==1
		
		sort y_bb1 
		
		gen cum_w =sum(aux99) if y_bb1>0&dep==1
		
		gen cumfreq=cum_w/tot_w
		
		egen cutter =cut(cumfreq), at(0(0.01)0.99 1)
		replace cutter=cutter[_n-1] if missing(cutter) &  y_bb1>0&dep==1
		gen per=cutter
		
		preserve
		collapse (min) p = per (mean) y_bb1 (count) obs = aux99 [fweight=expr] if y_bb1>0 &  dep==1, by(cutter)
		 
		export excel using "C:\Users\manue\Documents\Banco Central\Trabajo\tesis\Replicability\CASEN\2020\penn_2017.xlsx", firstrow(variables) sheet("bruto casen", modify)
...
drop tot_w aux99 cum_w cumfreq cutter per
		...
		egen double tot_w = total(expr) if y_ns1CN>0&dep==1
		gen aux99 = expr if y_ns1CN>0&dep==1
		
		sort y_ns1CN
		
		gen cum_w =sum(aux99) if y_ns1CN>0&dep==1
		
		gen cumfreq=cum_w/tot_w
		
		egen cutter =cut(cumfreq), at(0(0.01)0.99 1)
		replace cutter=cutter[_n-1] if missing(cutter) &  y_ns1CN>0&dep==1
		gen per=cutter
		
		preserve
		collapse (min) p = per (mean) y_ns1CN (count) obs = aux99 [fweight=expr] if y_ns1CN>0 &  dep==1, by(cutter)
		 
		export excel using "C:\Users\manue\Documents\Banco Central\Trabajo\tesis\Replicability\CASEN\2020\penn_2017.xlsx", firstrow(variables) sheet("neto cn", modify)
...
drop tot_w aux99 cum_w cumfreq cutter per
		...
		egen double tot_w = total(expr) if y_b1CN>0&dep==1
		gen aux99 = expr if y_b1CN>0&dep==1
		
		sort y_b1CN
		
		gen cum_w =sum(aux99) if y_b1CN>0&dep==1
		
		gen cumfreq=cum_w/tot_w
		
		egen cutter =cut(cumfreq), at(0(0.01)0.99 1)
		replace cutter=cutter[_n-1] if missing(cutter) &  y_b1CN>0&dep==1
		gen per=cutter
		
		preserve
		collapse (min) p = per (mean) y_b1CN (count) obs = aux99 [fweight=expr] if y_b1CN>0 &  dep==1, by(cutter)
		 
		export excel using "C:\Users\manue\Documents\Banco Central\Trabajo\tesis\Replicability\CASEN\2020\penn_2017.xlsx", firstrow(variables) sheet("bruto cn", modify)
...
ineqdeco y_bbb [w=expr] if  y_bbb>0 
	...
	putexcel L7 =`r(gini)', overwritefmt
	
sum y_bbb [w=expr] if  y_bbb>0, detail

	putexcel H7 =`r(mean)', overwritefmt
    putexcel I7 =`r(p50)', overwritefmt
drop aux*


replace y_gc1=y_gc1*`ipc'
bfmcorr using "C:\Users\manue\Documents\Magister\De Gregorio\Tesis\Bases\Encuestas\Finales\Segunda etrega\Replicabilidad\CASEN\gpinter 2017.xlsx", weight(expr) income(y_gc1) households(id_vivienda) taxunit(i) merg(${p_bfm} ) holdmar(dep cotiza trabajador contrib)
replace y_gc1=y_gc1/`ipc'
...
egen y_bbcn=rsum(y_b1cn y_b2cn y_b3cn y_bb4)
	...
	*****************REPORTAR FACTORES
	...
	putexcel g1 = ${year_casen} , overwritefmt
	sum f_recigc
	putexcel g2 =`r(mean)', overwritefmt
	sum f_cots
	putexcel g3 =`r(mean)', overwritefmt
	sum f_cots_i
	putexcel g4 =`r(mean)', overwritefmt
	sum f_rem
	putexcel g5 =`r(mean)', overwritefmt
	sum f_indep
	putexcel g6 =`r(mean)', overwritefmt
	sum f_cap
	putexcel g7 =`r(mean)', overwritefmt
	sum f_recigc_bfm
	putexcel g8 =`r(mean)', overwritefmt
	sum f_rembf
	putexcel g9 =`r(mean)', overwritefmt
	sum f_indepbf
	putexcel g10 =`r(mean)', overwritefmt
	sum f_capbfm
	putexcel g11 =`r(mean)', overwritefmt
	sum f_cotsbf
	putexcel g12 =`r(mean)', overwritefmt
	sum f_cots_ibf
	putexcel g13 =`r(mean)', overwritefmt
...
ineqdeco y_bbcn [w=cweight] if  y_bbcn>0 
	...
	putexcel M7 =`r(gini)', overwritefmt
	
sum y_bbcn [w=cweight] if  y_bbcn>0, detail

	putexcel J7 =`r(mean)', overwritefmt
    putexcel K7 =`r(p50)', overwritefmt

	
	

 drop tot_w  cum_w cumfreq cutter per
		
		egen double tot_w = total(cweight) if y_ns1cn>0&dep==1
		gen aux99 = cweight if y_ns1cn>0&dep==1
		...
		preserve
		gen int iweight =cweight
		collapse (min) p = per (mean) y_ns1cn (count) obs = aux99 [fweight=iweight] if y_ns1cn>0 &  dep==1, by(cutter)
		 
		export excel using "C:\Users\manue\Documents\Banco Central\Trabajo\tesis\Replicability\CASEN\2020\penn_2017.xlsx", firstrow(variables) sheet("net bfm cn", modify)
...
drop tot_w aux99 cum_w cumfreq cutter per
		...
		egen double tot_w = total(cweight) if y_b1cn>0&dep==1
		gen aux99 = cweight if y_b1cn>0&dep==1
		...
		preserve
		gen int iweight =cweight
		collapse (min) p = per (mean) y_b1cn (count) obs = aux99 [fweight=iweight] if y_b1cn>0 &  dep==1, by(cutter)
		 
		export excel using "C:\Users\manue\Documents\Banco Central\Trabajo\tesis\Replicability\CASEN\2020\penn_2017.xlsx", firstrow(variables) sheet("gross cn bfm", modify)
