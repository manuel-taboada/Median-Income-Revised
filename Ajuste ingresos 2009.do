...
************parámetros a ajustar tci tasa capital imputado
	...
	* Nombre del identificador de hogar;
	...
	global firmo 		=	"o25==1";
	global no_firmo 	=	"o25!=1";
	global no_boleteo	= 	"o22!=1 & o22!=2";
	global da_boleta	=	"o22==1 | o22==2";	
	global no_tiene_afp	=	"o32>5|o32==.";
	global cot_pension	=	"o32 >=1& o32<=5" ;
	...
	******************** Globales: agregados para ajustes a cuentas nacionales y registros administrativos;
	  global indep_cn = 9981244906804.37/`ipc';
  global capital_CN = 10392179382415.4/`ipc';
	global rec_sii = 1770669000000/`ipc';
	  
	global sal_cn = 36023099478003/`ipc';
	global cot_dep = 1312793000000/`ipc'; 

    global cot_indep = 58957000000/`ipc';
...
keep folio o* expr edad y* activ  numper  pco1
drop if edad<20
...
egen aux1 = rsum(y1 y3m1 y3m2 y3m3 y3m4 y3m5 y3m6 y3m7 y3m8 y3m9 y4m1 y4m2 y4m3 y4m4 y5m1 y5m2 y5m3 y5m4 y5m5 y5m6 y5m7 y5m8 y5m9 y5m10 y5m11 y6 y13m1)
    ...
    * Restamos ingresos en especies de los asalariados, ya que no pagan impuesto a la renta
	egen especies_em = rsum(y3m6 y3m7  y5m*) // detalles de estas variables en el libro de codigos 
...
* Generamos el ingreso de los independientes
	...
	replace y16=y16/12
	egen aux2 = rsum(y7 y8 y9 y10 y13m2 y16)
	...
	* egen aux1 = rsum(y260201c y2603c  yinv01 yinv03 ymon yorf yotp)
	egen aux3 = rsum(y21m1 y21m2 y21m3 y21m4 y21m5 y21m6)
	replace y_n4 = aux3
	...
	***ingresos totales del capital (dividendos, retiro de utilidades, intereses. Incluye arriendos que no están en la frontera de cn). Netos de impuesto.
	replace y14m1=y14m1/12
	replace y14m2=y14m2/12
	replace y14m3=y14m3/12
	
	replace y15m1=y15m1/12
	replace y15m2=y15m2/12
	egen cap_tot= rsum(y11m1 y11m2 y14m1 y14m2 y14m3 y15m1 y15m2)
...
ineqdeco y_bbb [w=expr] if  y_bbb>0 
	...
	putexcel L3 =`r(gini)', overwritefmt
	
sum y_bbb [w=expr] if  y_bbb>0, detail

	putexcel H3 =`r(mean)', overwritefmt
    putexcel I3 =`r(p50)', overwritefmt

drop aux*
drop if expr==.
****hay una obs
replace y_gc1=y_gc1*`ipc'
	bfmcorr using "C:\Users\manue\Documents\Magister\De Gregorio\Tesis\Bases\Encuestas\Finales\Segunda etrega\Replicabilidad\CASEN\gpinter 2009.xlsx", weight(expr) income(y_gc1) households(folio) taxunit(i) merg(${p_bfm} ) holdmar(dep cotiza trabajador contrib)
...
egen y_bbcn=rsum(y_b1cn y_b2cn y_b3cn y_bb4)
	...
	*****************REPORTAR FACTORES
	...
	putexcel c1 = ${year_casen} , overwritefmt
	sum f_recigc
	putexcel c2 =`r(mean)', overwritefmt
	sum f_cots
	putexcel c3 =`r(mean)', overwritefmt
	sum f_cots_i
	putexcel c4 =`r(mean)', overwritefmt
	sum f_rem
	putexcel c5 =`r(mean)', overwritefmt
	sum f_indep
	putexcel c6 =`r(mean)', overwritefmt
	sum f_cap
	putexcel c7 =`r(mean)', overwritefmt
	sum f_recigc_bfm
	putexcel c8 =`r(mean)', overwritefmt
	sum f_rembf
	putexcel c9 =`r(mean)', overwritefmt
	sum f_indepbf
	putexcel c10 =`r(mean)', overwritefmt
	sum f_capbfm
	putexcel c11 =`r(mean)', overwritefmt
	sum f_cotsbf
	putexcel c12 =`r(mean)', overwritefmt
	sum f_cots_ibf
	putexcel c13 =`r(mean)', overwritefmt
...
ineqdeco y_bbcn [w=cweight] if  y_bbcn>0 
	...
	putexcel M3 =`r(gini)', overwritefmt
	
sum y_bbcn [w=cweight] if  y_bbcn>0, detail

	putexcel J3 =`r(mean)', overwritefmt
    putexcel K3 =`r(p50)', overwritefmt
