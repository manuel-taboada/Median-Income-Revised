...
************parámetros a ajustar tci tasa capital imputado
	...
	* Nombre del identificador de hogar;
	...
	global no_tiene_afp	=	"o29!=1";
	global cot_pension	=	"o29 == 1" ;
	...
	******************** Globales: agregados para ajustes a cuentas nacionales y registros administrativos;
	global indep_cn = 13101751970420.7/`ipc';
  global capital_CN = 15608458040856.9/`ipc';
	global rec_sii = 2377361100000/`ipc';
	    
	global sal_cn = 43958255808061.6/`ipc';
	global cot_dep = 1565828000000/`ipc'; 

    global cot_indep = 57990000000/`ipc';
...
keep folio o* expr_r2 edad y* activ numper  pco1
drop if edad<20
...
rename expr_r2 expr
	...
	****a
replace y3am=y3am*30 if y3ap==1	
replace y3am=y3am*4 if y3ap==2	
replace y3am=y3am*2 if y3ap==3	
replace y3am=y3am/2 if y3ap==5
replace y3am=y3am/3 if y3ap==6
replace y3am=y3am/4 if y3ap==7
replace y3am=y3am/6 if y3ap==8
replace y3am=y3am/12 if y3ap==9

*b
replace y3bm=y3bm*30 if y3bp==1	
replace y3bm=y3bm*4 if y3bp==2	
replace y3bm=y3bm*2 if y3bp==3	
replace y3bm=y3bm/2 if y3bp==5
replace y3bm=y3bm/3 if y3bp==6
replace y3bm=y3bm/4 if y3bp==7
replace y3bm=y3bm/6 if y3bp==8
replace y3bm=y3bm/12 if y3bp==9

*c

replace y3cm=y3cm*30 if y3cp==1	
replace y3cm=y3cm*4 if y3cp==2	
replace y3cm=y3cm*2 if y3cp==3	
replace y3cm=y3cm/2 if y3cp==5
replace y3cm=y3cm/3 if y3cp==6

replace y3cm=y3cm/4 if y3cp==7
replace y3cm=y3cm/6 if y3cp==8
replace y3cm=y3cm/12 if y3cp==9

*d

replace y3dm=y3dm*30 if y3dp==1	
replace y3dm=y3dm*4 if y3dp==2	
replace y3dm=y3dm*2 if y3dp==3	
replace y3dm=y3dm/2 if y3dp==5
replace y3dm=y3dm/3 if y3dp==6

replace y3dm=y3dm/4 if y3dp==7
replace y3dm=y3dm/6 if y3dp==8
replace y3dm=y3dm/12 if y3dp==9


*e
replace y3em=y3em*30 if y3ep==1	
replace y3em=y3em*4 if y3ep==2	
replace y3em=y3em*2 if y3ep==3	
replace y3em=y3em/2 if y3ep==5
replace y3em=y3em/3 if y3ep==6

replace y3em=y3em/4 if y3ep==7
replace y3em=y3em/6 if y3ep==8
replace y3em=y3em/12 if y3ep==9


*f
replace y3fm=y3fm*30 if y3fp==1	
replace y3fm=y3fm*4 if y3fp==2	
replace y3fm=y3fm*2 if y3fp==3	
replace y3fm=y3fm/2 if y3fp==5
replace y3fm=y3fm/3 if y3fp==6

replace y3fm=y3fm/4 if y3fp==7
replace y3fm=y3fm/6 if y3fp==8
replace y3fm=y3fm/12 if y3fp==9




egen aux1 = rsum(y1a y3am y3bm y3cm y3dm y3em y3fm y4a y4b y4c y5a y5b y5c y5d y5e y5f y5g y5h y5i y5k y5k y5l y6 y11a y14a)
    ...
    * Restamos ingresos en especies de los asalariados, ya que no pagan impuesto a la renta
	egen especies_em = rsum(y3em y3dm y5*) // detalles de estas variables en el libro de codigos 
...
* Generamos el ingreso de los independientes
	...
	egen aux2 = rsum(y7a y8 y9 y10 y17)
	...
	* egen aux1 = rsum(y260201c y2603c  yinv01 yinv03 ymon yorf yotp)
	egen aux3 = rsum(y26a y26b y27am y27bm y27cm y27dm y27em y27fm)
	replace y_n4 = aux3
	
sum y_n4 [w=expr] if y_n4>0, detail
	...
	***ingresos totales del capital (dividendos, retiro de utilidades, intereses. Incluye arriendos que no están en la frontera de cn). Netos de impuesto.
	...
	egen cap_tot= rsum(y12a y12b y15a y15b y15c y16a y16b)
...
ineqdeco y_bbb [w=expr] if  y_bbb>0 
	...
	putexcel L4 =`r(gini)', overwritefmt
	
sum y_bbb [w=expr] if  y_bbb>0, detail

	putexcel H4 =`r(mean)', overwritefmt
    putexcel I4 =`r(p50)', overwritefmt
drop aux*


replace y_gc1=y_gc1*`ipc'
	bfmcorr using "C:\Users\manue\Documents\Magister\De Gregorio\Tesis\Bases\Encuestas\Finales\Segunda etrega\Replicabilidad\CASEN\gpinter 2011.xlsx", weight(expr) income(y_gc1) households(folio) taxunit(i) merg(${p_bfm} ) holdmar(dep cotiza trabajador contrib boleta)
replace y_gc1=y_gc1/`ipc'
...
egen y_bbcn=rsum(y_b1cn y_b2cn y_b3cn y_bb4)
	...
	*****************REPORTAR FACTORES
	...
	putexcel d1 = ${year_casen} , overwritefmt
	sum f_recigc
	putexcel d2 =`r(mean)', overwritefmt
	sum f_cots
	putexcel d3 =`r(mean)', overwritefmt
	sum f_cots_i
	putexcel d4 =`r(mean)', overwritefmt
	sum f_rem
	putexcel d5 =`r(mean)', overwritefmt
	sum f_indep
	putexcel d6 =`r(mean)', overwritefmt
	sum f_cap
	putexcel d7 =`r(mean)', overwritefmt
	sum f_recigc_bfm
	putexcel d8 =`r(mean)', overwritefmt
	sum f_rembf
	putexcel d9 =`r(mean)', overwritefmt
	sum f_indepbf
	putexcel d10 =`r(mean)', overwritefmt
	sum f_capbfm
	putexcel d11 =`r(mean)', overwritefmt
	sum f_cotsbf
	putexcel d12 =`r(mean)', overwritefmt
	sum f_cots_ibf
	putexcel d13 =`r(mean)', overwritefmt
	
putexcel set resultados_casen_stata.xlsx, sheet(Figure 10) modify
	
ineqdeco y_bbcn [w=cweight] if  y_bbcn>0 

	putexcel M4 =`r(gini)', overwritefmt
	
sum y_bbcn [w=cweight] if  y_bbcn>0, detail

	putexcel J4 =`r(mean)', overwritefmt
    putexcel K4 =`r(p50)', overwritefmt
